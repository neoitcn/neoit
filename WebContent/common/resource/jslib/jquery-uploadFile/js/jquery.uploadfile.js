...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            uploadButtonClass: /*"ajax-file-upload"*/'up-file-class',
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
            ...
            var fileInputStr = "<input class='ajax-file-upload-input' type='file' id='" + fileUploadId + "' name='" + s.fileName + "' accept='"+s.acceptFiles+"'/>";
            if(s.multiple) {
                if(s.fileName.indexOf("[]") != s.fileName.length - 2) // if it does not endwith
                {
                    s.fileName += "[]";
                }
                fileInputStr = "<input class='ajax-file-upload-input' type='file' id='" + fileUploadId + "' name='" + s.fileName + "' accept='"+s.acceptFiles+"' multiple/>";
            }            
        ...
        function createProgressDiv(obj, s) {
            ...
            this.abort = $("<span class='ajax-file-upload-red ajax-file-upload-abort " + obj.formGroup + "'>" + s.abortStr + "</span>").appendTo(this.statusbar).hide();
            this.cancel = $("<span class='ajax-file-upload-red ajax-file-upload-cancel "+obj.formGroup +"'>" + s.cancelStr + "</span>").appendTo(this.statusbar).hide();
            this.done = $("<span class='ajax-file-upload-green'>" + s.doneStr + "</span>").appendTo(this.statusbar).hide();
            this.download = $("<span class='ajax-file-upload-green'>"+s.downloadStr+"</span>").appendTo(this.statusbar).hide();            
            this.del = $("<span class='ajax-file-upload-red'>" + s.deletelStr + "</span>").appendTo(this.statusbar).hide();
			if(s.showQueueDiv)
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj) {
            ...
            var options = {
                ...
                beforeSubmit: function (formData, $form, options) {
                    ...
                    pd.statusbar.append("<div style='color:red;margin-top:5px;'>" + s.uploadErrorStr + "</div>");
                    pd.cancel.show()
                    form.remove();
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.showStatusAfterSuccess) {
                        if(s.showDone) {
                        	...
                        	pd.statusbar.slideUp("slow");//2015-11-06 add by wangxiaolei
                            pd.done.hide();
                        }
                        if(s.showDelete) {
                            pd.del.show();
